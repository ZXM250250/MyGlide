...
class PicDispatcherThread(//得到图片的具体执行者
    ...
    fun findBitmap(request: BitmapRequest):Bitmap?{    //三级储存获得图片的方法
        Log.i("图片测试","开始去内存找图片")
        val isfind = MemoryCache.getBitmapFromMemory(request.url)!=null
        Log.i("图片测试","是否能从内存中取到"+isfind)
        if (isfind){   //第一步从内存中去取
            return MemoryCache.getBitmapFromMemory(request.url)
            }
        ...
        if (img!=null){
            //有的话就加入内存当中
            img.let { MemoryCache.put(request.url, it) }
            return img
        }
       ...
       val bitmap = NetCache.getbitmap(request.url,request)
        ...
        if (bitmap != null) {
            MemoryCache.put(request.url,bitmap)
        }
        if (bitmap != null) {
